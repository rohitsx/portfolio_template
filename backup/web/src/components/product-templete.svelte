<script lang="ts">
	...
	let { products, title }: { products: Product[]; title: string } = $props();
	...
	<div class="relative w-full">
		...
		>
			{#each products as product (product.id)}
				<div
					class="bg-[#3c3836] p-5 rounded-lg shadow-md flex flex-col flex-none w-64"
				>
					...
					<p
						...
						class:line-clamp-3={!expandedDescriptions[product.id]}
					>
						{product.description}
					</p>
					{#if product.description.length > 100}
						<button
							class="text-[#b8bb26] text-sm mb-4 self-start hover:underline"
